...
namespace Hackathon.Boilerplate.Foundation.BusinessValueTracker.Workers
{
    public class RfmTrainingWorker : IDeferredWorker
    {
        ...
        {
            this._logger.LogWarning("RfmTrainingWorker.RunAsync");
        ...
        public async Task UpdateRfmFacets(RfmStatistics statistics, CancellationToken token)
        {
            using (IServiceScope scope = _serviceProvider.CreateScope())
            {
                using (var xdbContext = scope.ServiceProvider.GetService<IXdbContext>())
                {
                    ...
                    //{
                    //    var reference = new IdentifiedContactReference(Constants.XConnect.IdentificationSource, identifier.CustomerId.ToString());
